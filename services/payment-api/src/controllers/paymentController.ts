...
class PaymentController {
  ...
  async renderPaymentPage(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!transaction) {
        res.status(404).send(
          this.renderErrorPage(
            ...
            "The payment session you're trying to access doesn't exist or has expired."
          )
        );
        return;
      }
      ...
      res.status(500).send(
        this.renderErrorPage('Payment Error', 'An error occurred while loading the payment page.')
      );
    }
  }
  ...
  async completePayment(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Update transaction
      const mobileOrCard =
        paymentMethod === 'card' ? (cardNumber ? cardNumber.slice(-4) : null) : mobileNumber;
   ...
   */
  private processCardPayment(cardNumber: string | undefined): string {
  ...
  private renderErrorPage(title: string, message: string): string {
    return `
      ...
      <html>
      <head>
        <title>${title} - ${process.env.ORG_BRAND_NAME || 'Payment'}</title>
   ...
   * Render payment form
   */
  private renderPaymentForm(transaction: {
    checkout_id: string;
    ...
    return `
      ...
      <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>${process.env.ORG_BRAND_NAME || 'Payment'} - ${methodName}</title>
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          body {
            font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
          ...
          .brand-logo { font-size: 24px; font-weight: bold; }
          .amount-section {
            padding: 32px 24px;
            text-align: center;
            background: #f9fafb;
      ...
      <body>
        <div class="payment-card">
          <div class="payment-header">
            <div class="brand-logo">${process.env.ORG_BRAND_NAME || 'Payment'}</div>
  ...
  private getPaymentFormFields(paymentMethod: string): string {
    if (paymentMethod === 'card') {
      return `
  ...
  private getTestInfo(paymentMethod: string): string {
    if (paymentMethod === 'card') {
