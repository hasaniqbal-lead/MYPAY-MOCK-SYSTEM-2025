...
async function idempotencyMiddleware(req, res, next) {
    ...
    try {
        ...
        // Check if key exists
        const existingKey = await database_1.prisma.payoutIdempotencyKey.findUnique({
            where: {
                merchantId_key: {
                    merchantId,
                    key: idempotencyKey,
        ...
        if (existingKey) {
            ...
            if ((0, utils_1.isIdempotencyKeyExpired)(existingKey.expiresAt)) {
                // Delete expired key
                await database_1.prisma.payoutIdempotencyKey.delete({
        ...
        await database_1.prisma.payoutIdempotencyKey.create({
            data: {
                merchantId,
                key: idempotencyKey,
                requestHash,
        ...
        res.json = function (body) {
            ...
            database_1.prisma.payoutIdempotencyKey
                .update({
                where: {
                    merchantId_key: {
                        merchantId,
